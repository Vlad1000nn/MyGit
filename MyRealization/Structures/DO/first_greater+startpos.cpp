...
public:
    ...
    {
        return first_greater(1, 0, size, start, value);
    }

    int first_greater(int v, int tl, int tr, int start, int value)
    {
        if (tree[v] < value || tr <= start)
            return -1;
        ...
        int mid = (tl + tr) >> 1;
        int temp = first_greater(2 * v, tl, mid, start, value);
        return (temp == -1) ? first_greater(2 * v + 1, mid, tr, start, value) : temp;
    }
