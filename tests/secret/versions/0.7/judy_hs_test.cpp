...
public:
    ...
    template<typename StringType>
    PWord_t insert(StringType&& Index_, std::size_t len = -1)
    {
        if(len == -1)
            len = str_len_(Index_);

        prepare_str(Index_, len);
    ...
    template<typename StringType>
    int del(StringType&& Index_, std::size_t len = -1)
    {
        if(len == -1)
            len = str_len_(Index_);
    ...
    template<typename StringType>
    PWord_t get(StringType&& Index_, std::size_t len = -1) const
    {
        if(len == -1)
            len = str_len_(Index_);
    ...
    {
        const std::size_t len = str_len_(Index_);
    ...
    template<typename StringType>
    Word_t& at(StringType&& Index_, std::size_t len = -1) &
    {
        if(len == -1)
            len = str_len_(Index_);
    ...
    // const итераторы
    template<typename StringType>
    const Word_t& at(StringType&& Index_, std::size_t len = -1) const &
    {
        if(len == -1)
            len = str_len_(Index_);
        return at_impl_(std::forward<StringType>(Index_), len);
