...
void solve()
{
	...
	for (int step = 0; step < 75; ++step)
	{
		for (auto& it : ananas)
		{
			...
			if (current.isZero())
				need[1] += ananas[0];
			else if (current.get_len() % 2 == 0)
			{
				curr = current.to_string();
				left = curr.substr(0, isz(curr) / 2);
				int i = isz(curr) / 2;
				while (i < isz(curr) - 1 && curr[i] == '0')
					++i;
				right = curr.substr(i);
				need[left] += ananas[current];
				need[right] += ananas[current];
			}
			else
				need[current * 2024] += ananas[current];
