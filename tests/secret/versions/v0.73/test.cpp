...
{
//*******************************CONTAINERS*************************************//
    std::initializer_list<std::pair<const std::string, Word_t>> init_lst1 = {{"hello", 1}, {"world", 2}, {"test", 3}};
    
    std::vector<std::string> vec1 = {"apple", "banana", "cherry"}; 
    std::vector<std::string> vec2 = {"dog", "cat"};

    std::set<std::string> s1 = {"alpha", "beta", "gamma"};
    std::set<std::string> s2 = {"one", "two", "three"};

    std::array<std::string, 3> arr1 = {"first", "second", "third"};
    std::array<std::string, 2> arr2 = {"start", "end"};

    std::list<std::string> lst1 = {"list1", "list2", "list3"};
    std::list<std::string> lst2 = {"item1", "item2"};

    std::deque<std::string> deq1 = {"deque1", "deque2"};
    std::deque<std::string> deq2 = {"elem1", "elem2", "elem3"};

    std::forward_list<std::string> flst1 = {"fwd1", "fwd2"};
    std::forward_list<std::string> flst2 = {"forward1"};

    std::multiset<std::string> ms1 = {"multi", "multi", "set"};
    std::multiset<std::string> ms2 = {"test"};

    std::unordered_set<std::string> us1 = {"unordered", "set", "test"};
    std::unordered_set<std::string> us2 = {"hash", "table"}; 

    std::map<std::string, Word_t> mp1 = {{"key1", 10}, {"key2", 20}, {"key3", 30}};
    std::map<std::string, Word_t> mp2 = {{"a", 1}, {"b", 2}, {"c", 3}};

    std::multimap<std::string, Word_t> mmp1 = {{"dup", 1}, {"dup", 2}, {"unique", 3}};
    
    std::vector<std::pair<std::string, Word_t>> vecp1 = {{"vec1", 100}, {"vec2", 200}, {"vec3", 300}};

//*********************************JUDY SL***************************************//
    std::cout << std::endl << "=========== JUDY SL TESTS ===========" << std::endl;
...
std::cout<< "Basic constructor: ";
    ...
    assert(jsl1.size() == jsl1_1.size() && jsl1.size() == 0);
    assert(jsl1.empty() && jsl1_1.empty());

    judy::JudySL* pjsl1_2 = new judy::JudySL();
    judy::JudySL* pjsl1_3 = new judy::JudySL;

    assert(pjsl1_2->size() == pjsl1_3->size() && pjsl1_2->size() == 0);
    assert(pjsl1_2->empty() && pjsl1_3->empty());
    
    delete pjsl1_2;
    delete pjsl1_3;

    std::unique_ptr<judy::JudySL> pjsl1_4 = std::make_unique<judy::JudySL>();
    assert(pjsl1_4->size() == 0);
...
std::cout << "initializer_list constructor: ";
    ...
    judy::JudySL jsl2_1 = {};
    judy::JudySL jsl2_2 = {{"hello", 1}, {"world", 2}, {"test", 3}};

    assert(jsl2_1.size() == 0);

    for(auto val : init_lst1) assert(jsl2_2[val.first] == val.second);
    assert(jsl2_2.size() == init_lst1.size());        
...
std::cout << "iterator<pair> constructors: ";
    ...
    std::map<std::string, Word_t>::const_iterator mp2_it1 = mp2.begin(); ++mp2_it1; ++mp2_it1; 
    judy::JudySL jsl3_1(mp1.begin(), mp1.end());
    judy::JudySL jsl3_2(mp2.cbegin(), mp2_it1);

    for(auto [key, val] : mp1) assert(jsl3_1[key] == val);
    for(auto it = mp2.begin(); it != mp2_it1; ++it) assert(jsl3_2[it->first] == it->second);

    judy::JudySL jsl3_3(mmp1.begin(), mmp1.end());
    judy::JudySL jsl3_4(vecp1.begin(), vecp1.end());

    for(auto [key, val] : jsl3_3)
        std::cout << key << ": " << val << '\n';

    assert(jsl3_3["dup"] == 1 && jsl3_3["unique"] == 3);
    for(auto [key, val] : vecp1) assert(jsl3_4[key] == val);
...
std::cout << "copy constructor: ";
    ...
    judy::JudySL jsl4_1(jsl3_1);
    assert(jsl4_1 == jsl3_1);

    judy::JudySL jsl4_2(judy::JudySL{{"temp1", 10}, {"temp2", 20}});
    assert(jsl4_2["temp1"] == 10 && jsl4_2["temp2"] == 20);
...
std::cout << "operator= : ";
    ...
    judy::JudySL jsl4_3 = jsl4_1;
    assert(jsl4_3 == jsl4_1);
...
std::cout << "insert() func: ";
    ...
    judy::JudySL jsl5_1;
    PWord_t pv1 = jsl5_1.insert("test1"); *pv1 = 100;
    pv1 = jsl5_1.insert("test2"); *pv1 = 200;
    assert(jsl5_1.get("test1") && *jsl5_1.get("test1") == 100);
    assert(jsl5_1.get("test2") && *jsl5_1.get("test2") == 200);
...
std::cout << "get() func : ";
    ...
    assert(jsl5_1.get("test1") && jsl5_1.get("test2"));
    assert(!jsl5_1.get("notexist"));
...
std::cout << "del() func : ";
    ...
    assert(jsl5_1.del("test1") == 1);
    assert(jsl5_1.del("test1") == 0);
    assert(!jsl5_1.get("test1"));
...
std::cout << "first() func : ";
    ...
    jsl5_1.clear();
    jsl5_1["aaa"] = 1; jsl5_1["bbb"] = 2; jsl5_1["ccc"] = 3;

    std::pair<std::string, PWord_t> ans1 = jsl5_1.first("");
    assert(ans1.second && ans1.first == "aaa");
    ans1 = jsl5_1.first("zzz");
    assert(!ans1.second);
...
std::cout << "next() func : ";
    ...
    ans1 = jsl5_1.next("aaa");
    assert(ans1.second && ans1.first == "bbb");
    ans1 = jsl5_1.next("ccc");
    assert(!ans1.second);
...
std::cout << "last() func : ";
    ...
    ans1 = jsl5_1.last("zzz");
    assert(ans1.second && ans1.first == "ccc");
    ans1 = jsl5_1.last("a");
    assert(!ans1.second);
...
std::cout << "prev() func : ";
    ...
    ans1 = jsl5_1.prev("ccc");
    assert(ans1.second && ans1.first == "bbb");
    ans1 = jsl5_1.prev("aaa");
    assert(!ans1.second);
...
std::cout << "size() func: ";
    ...
    judy::JudySL jsl6_1;
    assert(jsl6_1.size() == 0);
    jsl6_1.insert("one"); jsl6_1.insert("two"); jsl6_1.insert("three");
    assert(jsl6_1.size() == 3);
    jsl6_1.del("one");
    assert(jsl6_1.size() == 2);
...
std::cout << "swap() func : ";
    ...
    judy::JudySL jsl6_3(jsl6_1);
    jsl6_1.swap(jsl6_2);
    assert(jsl6_3 == jsl6_2);
    jsl6_2.swap(jsl6_1);
    assert(jsl6_3 == jsl6_1);
...
std::cout << "count() func : ";
    ...
    assert(jsl6_1.count("two") == 1);
    assert(jsl6_1.count("notexist") == 0);
...
std::cout << "contains() func : ";
    ...
    assert(jsl6_1.contains("two") == true);
    assert(jsl6_1.contains("notexist") == false);
...
std::cout << "set_debug_mode() func : ";
    ...
    judy::JudySL::set_debug_mode(true);
    {
        judy::JudySL jsl7_1;
    }
    judy::JudySL::set_debug_mode(false);
    {
        judy::JudySL jsl7_2;
    }
...
std::cout << std::endl << "operator[] : ";
    ...
    judy::JudySL jsl8_1;
    jsl8_1["key1"] = 10; jsl8_1["key2"] = 20; jsl8_1["key3"] = 30;

    assert(jsl8_1["key1"] == 10);
    assert(jsl8_1["key2"] == 20);
    assert(jsl8_1["newkey"] == 0);

    jsl8_1["key1"] = 100;
    assert(jsl8_1["key1"] == 100);
...
std::cout << "at() : ";
    ...
    assert(jsl8_1.at("key1") == 100);
    assert(jsl8_1.at("key2") == 20);

    const judy::JudySL jsl8_2 = {{"const1", 1}, {"const2", 2}, {"const3", 3}};
    assert(jsl8_2.at("const1") == 1);
    assert(jsl8_2.at("const3") == 3);
...
std::cout << "iterators : ";
    ...
    judy::JudySL jsl8_3(jsl8_2);

    // begin(), cbegin()
    judy::JudySL::iterator jsl8_1_it1 = jsl8_1.begin();
    judy::JudySL::const_iterator jsl8_1_it2 = jsl8_1.begin();
    assert(jsl8_1_it2 == jsl8_1.cbegin());

    assert(jsl8_1_it1 != jsl8_3.begin());
    assert(jsl8_1_it2 != jsl8_3.cbegin());

    assert(jsl8_1_it1->first == "key1" && jsl8_1_it1->second == 100);
    assert(jsl8_1_it2->first == "key1" && jsl8_1_it2->second == 100);
    
    // iterator operator++, const_iterator operator++
    ++jsl8_1_it1; ++jsl8_1_it2;
    assert(jsl8_1_it1->first == "key2" && jsl8_1_it1->second == 20);
    assert(jsl8_1_it2->first == "key2" && jsl8_1_it2->second == 20);
    ...
    assert(jsl8_1_it1 == jsl8_1.end());
    assert(jsl8_1_it2 == jsl8_1.cend());

    assert(jsl8_1_it1 != jsl8_3.end());
    assert(jsl8_1_it2 != jsl8_3.cend());
...
/*
    ...
    // rbegin(), crbegin()
    judy::JudySL::reverse_iterator jsl8_1_rit1 = jsl8_1.rbegin();
    judy::JudySL::const_reverse_iterator jsl8_1_rit2 = jsl8_1.rbegin();
    assert(jsl8_1_rit2 == jsl8_1.crbegin());

    assert(jsl8_1_rit1 != jsl8_3.rbegin());
    assert(jsl8_1_rit2 != jsl8_3.crbegin());

    std::cout << jsl8_1_rit1->first << jsl8_1_rit1->second << '\n';
    const std::string sh1 = jsl8_1_rit1->first;
    assert(sh1 == "newkey");

    std::cout << "First: '" << jsl8_1_rit1->first << "'" << std::endl;
    std::cout << "First size: " << jsl8_1_rit1->first.size() << std::endl;
    for (char c : jsl8_1_rit1->first) {
    ...
    std::string temp(jsl8_1_rit1->first);  // создаем копию
    std::cout << "Copied string: '" << temp << "'\n";
    std::cout << "Compare result: " << (temp == "newkey") << "\n";

    assert(jsl8_1_rit1->first == "newkey"); 
    assert(jsl8_1_rit1->second == 0);
    assert(jsl8_1_rit2->first == "newkey"); assert(jsl8_1_rit2->second == 0);

    // reverse_iterator operator++, const_reverse_iterator operator++
    ++jsl8_1_rit1; jsl8_1_rit1++;
    ++jsl8_1_rit2; jsl8_1_rit2++;
    
    assert(jsl8_1_rit1->first == "key2" && jsl8_1_rit1->second == 20);
    assert(jsl8_1_rit2->first == "key2" && jsl8_1_rit2->second == 20);
    
    ++jsl8_1_rit1; jsl8_1_rit2++;

    // rend(), crend()

    assert(jsl8_1_rit1 == jsl8_1.rend());
    assert(jsl8_1_rit2 == jsl8_1.crend());
    
    assert(jsl8_1_rit1 != jsl8_3.rend());
    assert(jsl8_1_rit2 != jsl8_3.crend());
    ...
    assert(jsl8_1_rit2->first == "key2" && jsl8_1_rit2->second == 20);
*/  
    std::map<std::string, Word_t> vals = {{"key1", 100}, {"key2", 20}, {"key3", 30}, {"newkey", 0}};

    for(auto it : jsl8_1) assert(vals[it.first] == it.second);                        
    for(const auto& it : jsl8_1) assert(vals[it.first] == it.second);             
    for(auto [key, val] : jsl8_1) assert(vals[key] == val);  
    for(const auto& [key, val] : jsl8_1) assert(vals[key] == val); 
...
std::cout << "lower_bound() : ";
    ...
    judy::JudySL::iterator lb_it1 = jsl8_1.lower_bound("key2");
    assert(lb_it1->first == "key2");
    judy::JudySL::const_iterator lb_it2 = jsl8_1.lower_bound("key2");
    assert(lb_it2->first == "key2");
    
    lb_it1 = jsl8_1.lower_bound("zzz");
    assert(lb_it1 == jsl8_1.end());

    lb_it2 = jsl8_1.lower_bound("zzz");
    assert(lb_it2 == jsl8_1.cend());
...
std::cout << "upper_bound() : ";
    ...
    lb_it1 = jsl8_1.upper_bound("key2");
    assert(lb_it1->first == "key3");
    lb_it2 = jsl8_1.upper_bound("key2");
    assert(lb_it2->first == "key3");
    
    lb_it1 = jsl8_1.upper_bound("newkey");
    assert(lb_it1 == jsl8_1.end());

    lb_it2 = jsl8_1.upper_bound("newkey");
    assert(lb_it2 == jsl8_1.cend());
...
std::cout << "equal_range() : ";
    ...
    std::pair<judy::JudySL::iterator, judy::JudySL::iterator> lb_it3 = jsl8_1.equal_range("key2");
    assert(lb_it3.first->first == "key2" && lb_it3.first->second == 20 && lb_it3.second->first == "key3" && lb_it3.second->second == 30);
    std::pair<judy::JudySL::const_iterator, judy::JudySL::const_iterator> lb_it4 = jsl8_1.equal_range("key2");
    assert(lb_it4.first->first == "key2" && lb_it4.first->second == 20 && lb_it4.second->first == "key3" && lb_it4.second->second == 30);

    lb_it3 = jsl8_1.equal_range("newkey");
    assert(lb_it3.first->first == "newkey" && lb_it3.first->second == 0 && lb_it3.second == jsl8_1.end());
    lb_it4 = jsl8_1.equal_range("newkey");
    assert(lb_it4.first->first == "newkey" && lb_it4.first->second == 0 && lb_it4.second == jsl8_1.cend());
...
std::cout << "find() : ";
    ...
    judy::JudySL::iterator find_it1 = jsl8_1.find("key2");
    assert(find_it1->first == "key2" && find_it1->second == 20);
    judy::JudySL::const_iterator find_it2 = jsl8_1.find("key2");
    assert(find_it2->first == "key2" && find_it2->second == 20);

    find_it1 = jsl8_1.find("notexist");
    find_it2 = jsl8_1.find("notexist");
    assert(find_it1 == jsl8_1.end());
    assert(find_it2 == jsl8_1.cend());
...
std::cout << "erase() : ";
    ...
    find_it1 = jsl8_1.find("key2");
    jsl8_1.erase(find_it1);
    assert(!jsl8_1.get("key2"));

    find_it2 = jsl8_1.find("key1");
    jsl8_1.erase(find_it2);
    assert(!jsl8_1.get("key1"));

    std::size_t erase_int1 = jsl8_1.erase("key3");
    assert(erase_int1 == 1);

    for(auto [key, val] : vals) jsl8_1[key] = val;
    jsl8_1.erase(jsl8_1.begin(), jsl8_1.end());
    assert(jsl8_1.size() == 0);
    
    for(auto [key, val] : vals) jsl8_1[key] = val;
    jsl8_1.erase(jsl8_1.cbegin(), jsl8_1.cend());
    assert(jsl8_1.size() == 0);
...
std::cout << "merge() : ";
    ...
    judy::JudySL jsl9_1 = {{"a", 10}, {"b", 20}};
    judy::JudySL jsl9_2 = {{"c", 30}, {"d", 40}};

    jsl9_1.merge(jsl9_2);
    assert(jsl9_1.size() == 4 && jsl9_2.size() == 0);

    judy::JudySL jsl9_3 = {{"key1", 100}, {"key3", 300}, {"key5", 500}};
    judy::JudySL jsl9_4 = {{"key2", 200}, {"key3", 999}, {"key4", 400}}; 

    jsl9_3.merge(jsl9_4);

    assert(jsl9_3.size() == 5 && jsl9_4.size() == 1 && jsl9_4["key3"] == 999);

    judy::JudySL jsl9_5 = {{"single", 1}};
    judy::JudySL jsl9_6; 

    jsl9_5.merge(jsl9_6);
    
    assert(jsl9_5.size() == 1 && jsl9_5["single"] == 1 && jsl9_6.size() == 0);

    judy::JudySL jsl9_7; 
    judy::JudySL jsl9_8 = {{"merge1", 101}, {"merge2", 201}};

    jsl9_7.merge(jsl9_8);

    assert(jsl9_7.size() == 2 && jsl9_8.size() == 0);
...
std::cout << std::endl << "operator== : ";
    ...
    judy::JudySL jsl10_1, jsl10_2;
    for(auto val : {"test1", "test2", "test3"}) jsl10_1[val] = jsl10_2[val] = 42;
    assert(jsl10_1 == jsl10_2);
...
// swap(JudySL, JudySL)
std::cout << std::endl << "judy::swap(JudySL, JudySL) : ";

    jsl9_3 = jsl9_1;
    judy::swap(jsl9_1, jsl9_2);
    assert(jsl9_3 == jsl9_2);
...
std::cout << "operator<< and operator>> : ";
    ...
    judy::JudySL jsl11_1;
    jsl11_1["stream_test"] = 123;

    std::stringstream ss;
    ss << jsl11_1["stream_test"];
    assert(ss.str() == "123");
    
    ss.clear();
    ss.str("456");
    ss >> jsl11_1["stream_test"];
    assert(jsl11_1["stream_test"] == 456);
