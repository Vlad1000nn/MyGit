...
void topsort(const vvll& graph, int u,vll& visit,  vll& order)
{
    if (visit[u]) return;
    visit[u] = 1;

    for (auto& it : graph[u])
        topsort(graph, it, visit, order);
...
void solve()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        while (!q.empty())
        {
            ...
            for (auto& it : inversed[curr])
                q.push(it);
        }
        if (!isz(current)) continue;

        int minimum = INT_MAX, cnt{};
        for (auto& it : current)
            minimum = min(minimum, vec[it]);
