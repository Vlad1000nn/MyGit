...
int main()
{
	...
	unsigned int texture2 = loadTexture("textures/texture2.png");
	...
	const std::vector<float> condition2 = {
		4.0f,  1.0f, 3.0f,
		0.0f,  1.0f, 2.0f,
		2.0f,  1.0f, -0.5f,
		3.0f,  1.0f, 2.0f,
	...
	std::vector<float> vertices, vertices2;

	// calcullate normals to each side
	glm::vec3 normal = glm::vec3(1.0f);
	{
		auto v_push = [](const float* p, const int i, const glm::vec3& normal, const float x_texture, const float y_texture, std::vector<float>& vertices) {
		...
		auto p_gen = [](float* p, const int i, const unsigned int* indexes, const std::vector<float>& condition)
			{
				p[0] = condition[indexes[i] * 3];
				p[1] = condition[indexes[i] * 3 + 1];
				p[2] = condition[indexes[i] * 3 + 2];
			};

		glm::vec3 vec_1, vec_2;
		float p1[3], p2[3], p3[3];
		for (int i = 0; i < indexes_size; i += 3) {
			p_gen(p1, i, indexes, condition);
			p_gen(p2, i + 1, indexes, condition);
			p_gen(p3, i + 2, indexes, condition);
			...
			v_push(p1, indexes[i], normal, 1.0f, 1.0f, vertices);
			v_push(p2, indexes[i + 1], normal, 1.0f, 0.0f, vertices);
			v_push(p3, indexes[i + 2], normal, 0.0f, 0.0f, vertices);
		

			p_gen(p1, i, indexes, condition2);
			p_gen(p2, i + 1, indexes, condition2);
			p_gen(p3, i + 2, indexes, condition2);

			vec_1 = glm::vec3(p2[0] - p1[0], p2[1] - p1[1], p2[2] - p1[2]);
			vec_2 = glm::vec3(p3[0] - p2[0], p3[1] - p2[1], p3[2] - p2[2]);
			normal = glm::normalize(glm::cross(vec_1, vec_2));

			v_push(p1, indexes[i], normal, 1.0f, 1.0f, vertices2);
			v_push(p2, indexes[i + 1], normal, 1.0f, 0.0f, vertices2);
			v_push(p3, indexes[i + 2], normal, 0.0f, 0.0f, vertices2);
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * (int)vertices2.size(), vertices2.data(), GL_STATIC_DRAW);
	...
	// spot light settings
	const glm::vec3 sl_color = glm::vec3(0.0f, 0.0f, 0.0f);
