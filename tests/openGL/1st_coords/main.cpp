...
int main()
{
	...
	// orthographic projection; 1,2 -> left and right pyramid's coords, 3,4 -> close and far pyramid's sides
	// 5,6 -> dist to close and far
	...
	// Z-buffer, turn on TEST to not draw more depth vertices
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// do not forget to clear depth buffer
		...
		// view matrix moves scene forward (to move our cam little back)
		glm::mat4 view = glm::mat4(1.0f);
		// z+ look at us, x+ right, y+ up
		view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));

		// perspective projection; w -> x/w, y/w, z/w; factically we made prism to pyramid
		// arg 1: Field of View; 
		...
		glBindVertexArray(VAO);
			for (unsigned int cube = 0; cube < 10; ++cube) {
				// model matrix (from local to global)
